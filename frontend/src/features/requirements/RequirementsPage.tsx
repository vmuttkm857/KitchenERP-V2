...
export function RequirementsPage() {
  ...
  async function saveSnapshot(){setSavingSnapshot(true);setSnapshotMessage('');try{await apiRequest('/requirement-snapshots',{method:'POST',body:JSON.stringify({criteria:{menu_ids:menuIds,start_date:startDate||null,end_date:endDate||null}})});setSnapshotMessage('固定需求快照已建立')}catch(error){setSnapshotMessage(error instanceof Error&&error.message.includes('409')?'相同條件的固定快照已存在':'固定快照建立失敗')}finally{setSavingSnapshot(false)}}
  async function download(){setLoading(true);setError('');try{await apiDownload('/exports/requirements/xlsx',{method:'POST',body:JSON.stringify({menu_ids:menuIds,start_date:startDate||null,end_date:endDate||null})})}catch{setError('需求量 Excel 下載失敗')}finally{setLoading(false)}}
