...
function RecipeUnitSelect({value,onChange}:{value:string;onChange:(value:string)=>void}){
  const isListed=units.includes(value as (typeof units)[number])
  return <select value={value} onChange={event=>onChange(event.target.value)}>{!isListed&&<option value={value} disabled>{value}（目前值，請改選）</option>}{units.map(unit=><option key={unit} value={unit}>{unit}</option>)}</select>
}
...
export function RecipeEditor({dish,onClose}:{dish:Dish;onClose:()=>void}){
  ...
  function addIngredient(){const ingredient=ingredients.find(item=>item.id===selectedIngredient);if(!ingredient||!recipe||recipe.items.some(item=>item.ingredient_id===ingredient.id))return;setRecipe({...recipe,items:[...recipe.items,{ingredient_id:ingredient.id,ingredient_code:ingredient.code,ingredient_name:ingredient.name,ingredient_unit:ingredient.unit,supplier_name:ingredient.supplier_name,quantity:'0',unit:ingredient.unit,loss_rate:'0',sort_order:recipe.items.length+1,notes:null}]});setSelectedIngredient('');setDirty(true);setMessage('')}
