...
export function RecipeEditor({dish,onClose}:{dish:Dish;onClose:()=>void}){
  ...
  return <section className="recipe-editor-page"><nav className="recipe-breadcrumb" aria-label="麵包屑"><button className="back-link" onClick={requestClose}>← 返回菜色管理</button><span>菜色管理</span><span aria-hidden="true">›</span><span>{dish.name}</span><span aria-hidden="true">›</span><strong>標準配方</strong></nav><div className="section-heading"><div><p className="eyebrow">標準配方</p><h2>{dish.code}｜{dish.name}</h2>{dirty&&<span className="unsaved-indicator">有尚未儲存的變更</span>}</div></div>
    <div className="recipe-picker recipe-picker-filtered"><label>搜尋食材<input value={ingredientSearch} onChange={event=>{setIngredientSearch(event.target.value);setIngredientPage(1);setSelectedIngredient('')}}/></label><label>食材分類<select value={ingredientCategory} onChange={event=>{setIngredientCategory(event.target.value);setIngredientPage(1);setSelectedIngredient('')}}><option value="">全部分類</option>{categories.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>供應商<select value={ingredientSupplier} onChange={event=>{setIngredientSupplier(event.target.value);setIngredientPage(1);setSelectedIngredient('')}}><option value="">全部供應商</option>{suppliers.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="recipe-candidate">選擇食材<select disabled={ingredientLoading||!ingredients.length} value={selectedIngredient} onChange={event=>setSelectedIngredient(event.target.value)}><option value="">{ingredientLoading?'載入中…':ingredients.length?'請選擇':'沒有符合條件的食材'}</option>{ingredients.filter(candidate=>!recipe.items.some(item=>item.ingredient_id===candidate.id)).map(item=><option key={item.id} value={item.id}>{item.code}｜{item.name}｜{item.category_name}｜{item.supplier_name??'無供應商'}</option>)}</select></label><button type="button" onClick={addIngredient} disabled={!selectedIngredient}>加入配方</button><PaginationControls page={ingredientPage} pageSize={25} total={ingredientTotal} onPage={page=>{setIngredientPage(page);setSelectedIngredient('')}}/></div>
