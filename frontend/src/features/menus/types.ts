...
export interface DishOption { id:string; code:string; name:string; is_active:boolean }
export interface MenuDish { id?:string; dish_id:string; dish_code?:string; dish_name?:string; diner_count:number; notes:string|null; sort_order:number }
export interface MenuSlot { menu_day_id?:string; menu_date:string; menu_meal_type_id:string; notes:string|null; dishes:MenuDish[] }
export interface MenuAggregate { menu:Menu; dates:string[]; meal_types:MealType[]; slots:MenuSlot[] }
