...
export function MenuEditor({ menu, onClose }: { menu: Menu; onClose: () => void }) {
  ...
  const [columnMealId,setColumnMealId]=useState<string|null>(null)
  ...
  function addDish(slot: MenuSlot, dish: DishOption) {
    ...
    setMealDraftSlot({ ...slot, dishes: [...slot.dishes, { dish_id: dish.id, menu_meal_type_column_id: null, dish_code: dish.code, dish_name: dish.name, dish_category_name: dish.category_name, diner_count: 1, notes: null, sort_order: slot.dishes.length + 1 }] })
    setError('')
  }
  ...
  async function saveMealDraft() {
    ...
    try {
      const payload = Object.values(nextSlots).filter(slot => slot.menu_day_id || slot.notes || slot.dishes.length).map(slot => ({ ...slot, dishes: slot.dishes.map(({ id, dish_id, menu_meal_type_column_id, diner_count, notes, sort_order }) => ({ id, dish_id, menu_meal_type_column_id, diner_count, notes, sort_order })) }))
