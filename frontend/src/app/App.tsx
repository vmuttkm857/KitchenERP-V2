...
export function App() {
  ...
  const [page, setPage] = useState<'categories' | 'suppliers' | 'ingredients' | 'dishes' | 'recipe' | 'menus' | 'menu-editor' | 'requirements' | 'snapshots' | 'purchases' | 'kitchen'>('categories')
  ...
  return (
    <main className="app-shell">
      ...
      <nav className="app-nav"><button onClick={() => setPage('categories')}>分類</button><button onClick={() => setPage('suppliers')}>供應商</button><button onClick={() => setPage('ingredients')}>食材</button><button onClick={() => setPage('dishes')}>菜色與配方</button><button onClick={() => setPage('menus')}>菜單</button><button onClick={() => setPage('kitchen')}>廚房作業</button><button onClick={() => setPage('requirements')}>需求量預覽</button><button onClick={() => setPage('snapshots')}>固定需求快照</button><button onClick={() => setPage('purchases')}>正式採購</button></nav>
      <div className="workspace">{page === 'categories' && <CategoriesPage />}{page === 'suppliers' && <SuppliersPage />}{page === 'ingredients' && <IngredientsPage />}{page === 'dishes' && <DishesPage onEditRecipe={dish => { setRecipeDish(dish); setPage('recipe') }} />}{page === 'recipe' && recipeDish && <RecipeEditor dish={recipeDish} onClose={() => setPage('dishes')} />}{page === 'menus' && <MenusPage onOpen={menu => { setEditingMenu(menu); setPage('menu-editor') }} />}{page === 'menu-editor' && editingMenu && <MenuEditor menu={editingMenu} onClose={() => setPage('menus')} />}{page === 'kitchen' && <KitchenOperationsPage/>}{page === 'requirements' && <RequirementsPage />}{page === 'snapshots' && <SnapshotsPage onPurchase={id=>{setPurchaseId(id);setPage('purchases')}}/>}{page === 'purchases' && <PurchasesPage initialId={purchaseId}/>}</div>
    </main>
  )
}
