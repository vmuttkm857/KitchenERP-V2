...
export function App() {
  ...
  const [page, setPage] = useState<'categories' | 'suppliers' | 'ingredients' | 'dishes' | 'recipe' | 'menus' | 'menu-editor' | 'requirements' | 'snapshots'>('categories')
  ...
  return (
    <main className="app-shell">
      ...
      <nav className="app-nav"><button onClick={() => setPage('categories')}>分類</button><button onClick={() => setPage('suppliers')}>供應商</button><button onClick={() => setPage('ingredients')}>食材</button><button onClick={() => setPage('dishes')}>菜色與配方</button><button onClick={() => setPage('menus')}>菜單</button><button onClick={() => setPage('requirements')}>需求量預覽</button><button onClick={() => setPage('snapshots')}>固定需求快照</button></nav>
