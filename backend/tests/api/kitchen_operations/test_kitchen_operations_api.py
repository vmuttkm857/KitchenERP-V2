...
def kitchen_fixture(client,headers):
    ...
    recipes=[
        ...
        {"items":[{"ingredient_id":ingredients[3]["id"],"quantity":"1","unit":"個","loss_rate":"0","sort_order":1}]},
    ]
...
def test_known_answer_hierarchy_decimal_notes_sort_and_units(client,db_session):
    headers=auth(client,db_session);menu,lunch,dinner,_,_=kitchen_fixture(client,headers)
...
def test_date_meal_filter_raw_display_and_summary(client,db_session):
    headers=auth(client,db_session);menu,lunch,_,_,_=kitchen_fixture(client,headers)
...
def test_inactive_ingredient_and_dish_warn_without_supplier_or_price_validation(client,db_session):
    headers=auth(client,db_session);menu,_,_,dishes,ingredients=kitchen_fixture(client,headers)
...
def test_auth_query_budget_read_only_and_no_snapshot_purchase_dependency(client,db_session):
    ...
    headers=auth(client,db_session);menu,*_=kitchen_fixture(client,headers);statements=[]
