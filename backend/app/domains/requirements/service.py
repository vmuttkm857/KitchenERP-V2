...
class RequirementService:
    ...
    def calculate(self,criteria):
        ...
        rows,anomalies=calculate_requirement_rows(source)
        ...
        return {"criteria":criteria,"source_menus":menus,"rows":rows,"supplier_groups":groups,"known_estimated_cost":known,"total_estimated_cost":None if incomplete else known,"anomalies":anomalies,"anomaly_summary":{"total":len(anomalies),"errors":sum(a["severity"]=="error" for a in anomalies),"warnings":sum(a["severity"]=="warning" for a in anomalies)}}
