...
class KitchenOperationsService:
    ...
    def calculate(self,criteria):
        ...
        meal_type_columns=self.repository.menu_columns(criteria.menu_id)
        ...
        return {"criteria":criteria,"menu":menu,"days":days,"meal_type_columns":meal_type_columns,"ingredient_summary":summary,"supplier_summary":supplier_summary,"anomalies":anomalies,"anomaly_summary":{"total":len(anomalies),"errors":sum(a["severity"]=="error" for a in anomalies),"warnings":sum(a["severity"]=="warning" for a in anomalies)}}
